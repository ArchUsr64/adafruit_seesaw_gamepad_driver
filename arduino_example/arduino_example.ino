...
void set_pin_mode_input_pullup(uint32_t pins) {
  uint8_t buffer[6] = {
    SEESAW_GPIO_BASE, SEESAW_GPIO_DIRCLR_BULK, (uint8_t)(pins >> 24),
    (uint8_t)(pins >> 16),(uint8_t)(pins >> 8), (uint8_t)pins
  };
  write(buffer, 6);
...
void setup() {
  ...
  {
    uint8_t buffer[] = {SEESAW_STATUS_BASE, SEESAW_STATUS_SWRST, 0xFF};
    write(buffer, 3);
    delay(10);
  }
...
uint32_t digital_read_bulk(uint32_t pins) {
  ...
  uint8_t buffer[4];
  read(buffer, 4);
  uint32_t ret = ((uint32_t)buffer[0] << 24) | ((uint32_t)buffer[1] << 16) |
